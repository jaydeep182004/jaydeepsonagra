   ...
   if($_POST["name"]==""){
    ...
    if($name!="" && $password!="" && $state!="" && $gender!="" && $checkbox>=2 && $checkbox<=4 && $imagevalue!=""){
      if(isset($_POST['submit'])){
        $sql = "INSERT INTO tabledata(name,password,gender,state,hobbies,image)
    ...
    if($name!="" && $password!="" && $state!="" && $gender!="" && $checkbox>=2 && $checkbox<=4){
      ...
      $update="UPDATE tabledata set name='$name',password='$password',state='$state',gender='$gender',hobbies='$allvalue'";
